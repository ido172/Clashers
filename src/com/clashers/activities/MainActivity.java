...
public class MainActivity extends FragmentActivity implements INearUsersParent {
	...
	private static MainActivity thisActivityInstanse = null;

	public static MainActivity getActivityInsttance() {
		return thisActivityInstanse;
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		thisActivityInstanse = this;

		metrics = getResources().getDisplayMetrics();
		initServices();

		if (!isOnline()) {
			findViewById(R.id.no_connection_container).setVisibility(
					View.VISIBLE);
			...
			retryBtn.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					if (isOnline()) {
						GCMRegister();
						findViewById(R.id.no_connection_container)
							...
							new Request.GraphUserCallback() {
								...
								public void onCompleted(GraphUser user,
									...
									if (user != null) {
										...
										editor.putString(Data.PREFS_UID,
												user.getId());
										editor.putString(Data.PREFS_NAME,
												user.getName());
										// Commit the edits!
										editor.commit();

										Log.d("SUCCESS FACEBOOK LOGIN", "ID IS"
												+ user.getId());
										initServices();
										getNearUsers();
	...
	public void UpdateUsersNearBy(ArrayList<UserDataFromServer> usersDataList) {
		if (usersDataList == null) {
			...
			Collections.sort(usersDataList,
					new Comparator<UserDataFromServer>() {

						@Override
						public int compare(UserDataFromServer lhs,
								UserDataFromServer rhs) {
			...
			for (int i = 0; i < usersDataList.size(); i++) {
				...
				currUser.setSongData(new SongData(currUserData.getSong(),
						currUserData.getArtist(), currUserData.getAlbum(), "0"));
